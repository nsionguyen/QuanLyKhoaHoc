...
import com.ntn.quankykhoahoc.Services.CauHoiServices;
import com.ntn.quankykhoahoc.Services.DapAnServices;
import com.ntn.quankykhoahoc.Services.NguoiDungServices;
import com.ntn.quankykhoahoc.Services.NopBaiServices;
...
public class BaiTapController implements Initializable {
    ...
    private static final String TIMER_KEY = "remainingTime3";
    private static final String QUESTION_KEY = "question1_";
    private static Preferences preferences = Preferences.userRoot().node(BaiTapController.class.getName());

// Phương thức để load câu hỏi và đáp án, xử lý tạo động radio buttons
    public void loadCauHoi() {
        CauHoiServices loadCauHoi = new CauHoiServices();
        try {
            this.cauhoi = loadCauHoi.getCauHoiTheoBaiTapID(baiTapID);
        } catch (SQLException ex) {
            Logger.getLogger(BaiTapController.class.getName()).log(Level.SEVERE, null, ex);
        }
    ...
    public void load() {
        try {
            ...
            DapAnServices da = new DapAnServices();
...
//        System.out.println("Key - Value: " + entry.getKey() + " - " + entry.getValue());
    ...
    public void submit() throws SQLException {
        ...
        for (Map.Entry<Integer, String> entry : cauHoiMap.entrySet()) {
            ...
            DapAnServices dapAnServices = new DapAnServices();
        ...
        NopBaiServices nopBaiServices = new NopBaiServices();
        nopBaiServices.nopBaiServices(id, baiTapID, ketQua);
